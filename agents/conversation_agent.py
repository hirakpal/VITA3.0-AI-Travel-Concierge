...
class ConversationManager(BaseAgent):
    ...
    # Main Router
    # =====================================================
    def run(self, state, message: str) -> AgentResponse:
        # --------------------------------------------
        # Store user message
        # --------------------------------------------
        state.set_user_input(message)
        self.audit(state, "Received user message.")
        ...
        clean_text = guard.data.get("clean_text", message)
        ...
        self.audit(state, f"Conversation routed to {route}")
        ...
        # Ask Gemini
        # --------------------------------------------
        answer = self.ask(self.system_prompt(), reply)
    ...
    # Router
    # =====================================================
    def detect_route(self, text: str) -> str:
        text = text.lower()

        # ---------------- Planner ----------------
        planner_keywords = ["plan", "itinerary", "schedule", "trip", "vacation", "holiday"]
        if any(k in text for k in planner_keywords):
            return "PLANNER"

        # ---------------- Approval ----------------
        if any(k in text for k in ["approve", "confirm", "book", "go ahead"]):
            return "APPROVAL"

        # ---------------- Map ----------------
        if any(k in text for k in ["map", "nearby", "around", "location", "city"]):
            return "MAP"

        # ---------------- Discovery ----------------
        if any(k in text for k in ["recommend", "suggest", "looking", "want", "love"]):
            return "DISCOVERY"
...
"""
    ...
    # Prompt Builders
    # =====================================================
    def general_prompt(self, message):
        return f"""
Traveller says
{message}
...
"""
    ...
    def discovery_prompt(self, message):
        return f"""
...
"""
    ...
    def planner_prompt(self, message):
        return f"""
